...
func (db *SQLiteDB) BeginTransaction() error {
	db.txMu.Lock()
...
func (db *SQLiteDB) CommitTransaction() error {
	db.txMu.Lock()
	defer db.txMu.Unlock()
...
func (db *SQLiteDB) RollbackTransaction() error {
	db.txMu.Lock()
	defer db.txMu.Unlock()
...
func (db *SQLiteDB) BatchInsertInTransaction(database, table string, rows []map[string]interface{}) error {
	if len(rows) == 0 {
		return nil
	}

...
// TemplateManagerDB 使用数据库存储的模板管理器（支持 MySQL/PostgreSQL）
type TemplateManagerDB struct {
	storage   storage.StorageInterface
	templates map[string]*ConfigTemplate
	mu        sync.RWMutex
}

// NewTemplateManagerDB 创建使用数据库存储的模板管理器
func NewTemplateManagerDB(storage storage.StorageInterface) (*TemplateManagerDB, error) {
	tm := &TemplateManagerDB{
		storage:   storage,
...
// SaveTemplate 保存模板
func (tm *TemplateManagerDB) SaveTemplate(name, description, tableName string, config *TableConfig) (string, error) {
	tm.mu.Lock()
	defer tm.mu.Unlock()
...
// GetTemplate 获取模板
func (tm *TemplateManagerDB) GetTemplate(templateID string) (*ConfigTemplate, error) {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
...
func (tm *TemplateManagerDB) GetAllTemplates() []*ConfigTemplate {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
...
func (tm *TemplateManagerDB) GetTemplatesByTable(tableName string) []*ConfigTemplate {
	tm.mu.RLock()
	defer tm.mu.RUnlock()
...
// UpdateTemplate 更新模板
func (tm *TemplateManagerDB) UpdateTemplate(templateID, name, description string, config *TableConfig) error {
	tm.mu.Lock()
	defer tm.mu.Unlock()
...
// DeleteTemplate 删除模板
func (tm *TemplateManagerDB) DeleteTemplate(templateID string) error {
	tm.mu.Lock()
	defer tm.mu.Unlock()
...
func (tm *TemplateManagerDB) LoadTemplates() error {
	tm.mu.Lock()
	defer tm.mu.Unlock()

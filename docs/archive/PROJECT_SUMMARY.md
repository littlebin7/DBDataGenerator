...
这是一个基于 Go + Vue3 开发的数据库数据生成工具，参考 Navicat 的数据生成功能，支持多种数据库类型，提供 Web 界面进行可视化管理。

**项目目标**：帮助开发者和测试人员快速生成测试数据，支持批量生成多线程处理、配置模板保存等功能。
...
- **存储方式**：支持多种存储后端（文件存储、SQLite、MySQL、PostgreSQL）
...
DBDataGenerator/
 ...
 internal/
   ...
   database/                   # 数据库抽象层
     ...
     dameng.go              # 达梦数据库实
     connection_manager.go  # 连接管理器文件存储
     connection_manager_sqlite.go  # 连接管理器SQLite可选
   generator/                  # 数据生成引擎
     types.go               # 类型定义
     rules.go                # 生成规则实现
     engine.go               # 生成引擎核心
     utils.go                # 工具函数
     template_manager.go    # 模板管理器文件存储
     template_manager_sqlite.go  # 模板管理器SQLite可选
   task/                       # 任务管理
     types.go                # 任务类型定义
     manager.go              # 任务管理
     pool.go                 # 工作池多线程
   ...
   src/
    stores/                 # Pinia 状态管理
     ...
     views/                  # 页面视图
       ...
       DatabaseSelectView.vue  # 数据表选择（树状结构）
       TaskView.vue            # 任务管理
...
- 连接配置包含名称类型主机端口用户名密码数据库名SSL模式字符集

**API 端点**
- `POST /api/connect/test` - 测试连接带5秒超时
- `POST /api/connect` - 添加连接必须先测试成功
...
**API 端点**
- `GET /api/databasesconnection_id=xxx` - 获取数据库列
- `GET /api/tablesdatabase=xxx&connection_id=xxx` - 获取表列
- `GET /api/table/:name/schemadatabase=xxx&connection_id=xxx` - 获取表结
...
1. **随机字符* (`random_string`)
   - 配置最小长度最大长度字符集all/letters/numbers/symbols
...
8. **函数** (`function`)
   - 内置函数UUID(), NOW(), RAND(), RAND_INT(min, max) 
...
- `GET /api/templatestable_name=xxx` - 获取模板列表
...
#### 获取数据库列
```
GET /api/databasesconnection_id=xxx
...
#### 获取表列
```
GET /api/tablesdatabase=xxx&connection_id=xxx
...
#### 获取表结
```
GET /api/table/:name/schemadatabase=xxx&connection_id=xxx
...
#### 获取模板列表
```
GET /api/templatestable_name=users
...
### 主键处理
- 如果字段是主键且是自增类型使用递增规则
- 如果字段是主键且UUID 类型使UUID() 函数
- 如果字段是主键且是字符串类型使UUID() 函数
...
4. **暂停/恢复**
   - 暂停设置状态为 `paused`停止分配新批次
   - 恢复设置状态为 `running`继续分配批
...
- `connections.json` - 连接配置运行时生成已加入 .gitignore
  - `connections` - 连接配置密码加密运行时生成已加入 .gitignore
...
- [x] 数据库连接管理文件存储
- [x] 多数据库类型支持PostgreSQLMySQLMariaDB达梦
- [x] 连接测试带5秒超时
- [x] 数据库列表获
- [x] 表列表获
- [x] 表结构解
...
- [x] 配置模板管理文件存储
- [x] REST API 接口
- [x] WebSocket 支持已实现并集成到 Pinia store
...
- [ ] 编辑连接 API目前只有前端UI
- [ ] 重新连接 API目前只有前端UI
- [x] Pinia 状态管理迁移已完成所有组件迁移
- [x] WebSocket 实时更新任务状态已实现并集成到 store
- [ ] 任务日志记录
- [ ] 数据生成预览功能
- [ ] 更多内置函数支持
...
**支持的存储类型**
- 文件存储JSON格式- 适合单机部署和开发环境
- SQLite - 轻量级数据库适合小型项目
- MySQL/MariaDB - 适合生产环境支持高并发
...
- 后端已实现现 WebSocket Hub
- 前端已集成 WebSocket通过 Pinia store 实时更新任务状态
- 所有使用 TaskStore 的组件自动获得实时更新

### 4. 密码存储

- 连接密码使用 AES-256-GCM 加密存储SQLite 数据库中
- 生产环境建议加密存储

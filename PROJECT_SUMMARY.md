...
- **存储方式**：文件存储（JSON格式，避免 SQLite CGO 依赖问题）
...
│   │   │   ├── DatabaseSelectView.vue  # 数据库/表选择（树状结构）
│   │   │   ├── TaskView.vue            # 任务管理
...
├── go.mod                          # Go 模块定义
├── go.sum                          # Go 依赖校验
├── connections.json                # 连接配置（运行时生成）
├── templates.json                  # 模板配置（运行时生成）
├── README.md                       # 项目说明
├── DESIGN.md                       # 设计文档
...
- 连接配置包含：名称、类型、主机、端口、用户名、密码、数据库名、SSL模式、字符集等
...
**API 端点**：
- `GET /api/databases?connection_id=xxx` - 获取数据库列表
- `GET /api/tables?database=xxx&connection_id=xxx` - 获取表列表
- `GET /api/table/:name/schema?database=xxx&connection_id=xxx` - 获取表结构
...
8. **函数** (`function`)
   - 内置函数：UUID(), NOW(), RAND(), RAND_INT(min, max) 等
...
- 如果字段是主键且是 UUID 类型：使用 UUID() 函数
- 如果字段是主键且是字符串类型：使用 UUID() 函数
...
database:
  ...
  connection_max_lifetime: 3600  # 秒
...
- `templates.json` - 模板配置（运行时生成，已加入 .gitignore）
